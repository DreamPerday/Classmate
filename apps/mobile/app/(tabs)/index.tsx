...
import{applyAnalysis,createCourse,createSession,getDashboard,getSessionSummary,getSessionTranscripts,insertCapturedTranscript,insertTranscript,saveSessionSummary,setActiveCourse,setActiveSession}from"@/services/database";
...
export default function ClassroomScreen(){
  ...
  async function toggleCapture(){
    try{
      ...
      if(!status.modelReady)throw new Error("请先下载本地中文识别模型");
  ...
  return <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
    ...
    <View style={styles.selectorBar}>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.pillRow}>
        {data.courses.map(course=><Pressable key={course.id} onPress={()=>switchCourse.mutate(course.id)} style={[styles.pill,course.id===data.course.id&&styles.pillActive]}><Text style={[styles.pillText,course.id===data.course.id&&styles.pillTextActive]}>{course.name}</Text></Pressable>)}
    ...
    <View style={styles.selectorBar}>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.pillRow}>
        {data.sessions.map(session=><Pressable key={session.id} onPress={()=>switchSession.mutate(session.id)} style={[styles.pill,session.id===data.session.id&&styles.pillActive]}><Text style={[styles.pillText,session.id===data.session.id&&styles.pillTextActive]}>{session.title}</Text></Pressable>)}
    ...
    {status.supported&&!status.modelReady&&<View style={styles.localModel}>
      ...
      <View style={styles.localModelBody}><Text style={styles.localModelTitle}>本地中文识别模型</Text><Text style={styles.localModelText}>{modelProgress?modelProgressLabel(modelProgress):"约 42 MB，仅保存在设备内"}</Text></View>
    ...
    {data.transcripts.length>0&&<View>
      <View style={styles.sectionHeader}><Text style={styles.sectionTitle}>本课摘要</Text>{summary.data&&!generateSummary.isPending&&<Pressable accessibilityRole="button" accessibilityLabel="重新生成摘要" hitSlop={8} onPress={()=>generateSummary.mutate(data.session.id)}><Ionicons name="refresh" size={17} color={colors.primary}/></Pressable>}</View>
      {generateSummary.isPending?<View style={styles.summaryBox}><Text style={styles.summaryText}>正在依据课堂字幕生成摘要…</Text></View>:summary.data?<View style={styles.summaryBox}><Text style={styles.summaryText}>{summary.data.contentMd}</Text></View>:<Pressable onPress={()=>generateSummary.mutate(data.session.id)} style={({pressed})=>[styles.summaryGenerate,{opacity:pressed?0.5:1}]}><Ionicons name="sparkles-outline" size={16} color={colors.primary}/><Text style={styles.summaryGenerateText}>生成本课摘要</Text></Pressable>}
    </View>}
    ...
    <View style={styles.sectionHeader}><Text style={styles.sectionTitle}>最近字幕</Text><Text style={styles.sectionMeta}>{data.transcripts.length} 条</Text></View>
    {data.transcripts.length?data.transcripts.slice(0,10).map(item=><View key={item.id} style={styles.transcript}><Text style={styles.time}>{formatMs(item.startMs)}</Text><Text style={styles.transcriptText}>{item.text}</Text></View>):<EmptyState title="还没有课堂字幕" detail="开始系统捕获或手工添加一段文字后会出现在这里。" icon="mic-outline"/>}
    <View style={styles.sectionHeader}><Text style={styles.sectionTitle}>课堂理解</Text></View>
