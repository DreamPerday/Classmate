...
import{checkReportReadiness,deleteReport,getReportBounds,getReports,reportSummaryContext,saveReport}from"@/services/database";
import{generateLearningReport}from"@/services/ai";
import{exportReportDocx,exportReportMarkdown,exportReportPdf}from"@/services/report-export";
...
export default function ReportsScreen(){
  ...
  const generate=useMutation({
    mutationFn:async()=>{
      validateScope(scope);
      const readiness=await checkReportReadiness(scope);
      if(!readiness.ready){
        const missingList=readiness.missing.map(m=>`第${m.dayIndex}天 ${m.title}`).join("、");
        throw new Error(`以下课次尚未生成摘要，请先在课堂页生成：${missingList}`);
      }
      const summaries=await reportSummaryContext(scope);
      if(!summaries.length)throw new Error("所选日期范围内没有课堂记录");
      const context=summaries.map((row,index)=>`[证据${index+1}] 第${row.dayIndex}天 ${row.title} · ${String(row.createdAt).slice(0,10)}\n${row.contentMd}`).join("\n\n");
      const content=await generateLearningReport(scope,context,summaries.length);
      ...
      return saveReport(scope,content,summaries.length);
    },
    onSuccess:async report=>{setExpanded(report.id);await queryClient.invalidateQueries({queryKey:["mobile-reports"]});},
  });
  const removeReport=useMutation({mutationFn:(id:string)=>deleteReport(id),onSuccess:()=>queryClient.invalidateQueries({queryKey:["mobile-reports"]})});

  async function exportFile(report:LearningReport,kind:"docx"|"pdf"|"md"){
    setExportError("");setExporting(`${report.id}:${kind}`);
    try{
      if(kind==="docx")await exportReportDocx(report);
      else if(kind==="pdf")await exportReportPdf(report);
      else await exportReportMarkdown(report);
    }catch(error){setExportError(message(error));}finally{setExporting("");}
  }
  function confirmDeleteReport(id:string,title:string){
    Alert.alert("删除报告",`确定删除「${title}」？此操作不可撤销。`,[{text:"取消",style:"cancel"},{text:"删除",style:"destructive",onPress:()=>removeReport.mutate(id)}]);
  }
